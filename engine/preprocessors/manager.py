# -*- coding: utf-8 -*-
"""The operating system plugins manager."""
...
from engine.preprocessors import interface
from engine.preprocessors import logger
...
class PreprocessPluginsManager(object):
    ...
    def RegisterPlugin(cls, plugin_class):
        ...
        os_plugin = plugin_class()

        cls._plugins[name] = os_plugin

        if isinstance(os_plugin, interface.FileSystemArtifactPreprocessorPlugin):
            cls._file_system_plugins[name] = os_plugin

        elif isinstance(os_plugin, interface.KnowledgeBasePreprocessorPlugin):
            cls._knowledge_base_plugins[name] = os_plugin

        elif isinstance(os_plugin, interface.WindowsRegistryKeyArtifactPreprocessorPlugin):
            cls._windows_registry_plugins[name] = os_plugin

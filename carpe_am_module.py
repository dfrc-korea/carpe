...
class CARPE_AM:
    ...
    def ParseFilesystem(self):
        ...
        query = "SELECT par_id, sector_size, start_sector, par_size FROM partition_info WHERE evd_id='" + self.evd_id + "';"
        ...
        for par in par_info:
            ...
            par_size = int(str(par[3]))

            # for microsoft reserved partition
            if par_size == 134217728:
                continue
    ...
    def SysLogAndUserData_Analysis(self):
        ...
        for par_info in par_infos:
            ...
            # 전체돌리기 200311
            #subprocess.call(['python3.6', '/home/barley/CARPE/plaso_tool/carpe_l2t.py', '--no_vss', '--hashers', 'None', storage_path, self.path, p_name])
            # 빠르게 artifact만 돌리기
            subprocess.call(['python3.6', '/home/barley/CARPE/plaso_tool/carpe_l2t.py', '--artifact-filters',
                             'WindowsActivitiesCacheDatabase, WindowsAMCacheHveFile, WindowsAppCompatCache, WindowsApplicationCompatibilityInstalledShimDatabases, WindowsApplicationCompatibilityShimDatabaseMappings, WindowsApplicationCompatibilityShims, WindowsAvailableTimeZones, WindowsComputerName, WindowsCurrentVersion, WindowsEventLogs, WindowsEventLogApplication, WindowsEventLogSecurity, WindowsEventLogSystem, WindowsXMLEventLogApplication, WindowsXMLEventLogSecurity, WindowsXMLEventLogSysmon, WindowsXMLEventLogSystem, WindowsXMLEventLogTerminalServices, WindowsInstallationDateTime, WindowsPrefetchFiles, WindowsProductName, WindowsProgramsCache, WindowsProgramsCacheJumpLists, WindowsRecentFileCacheBCF, WindowsRecycleBin, WindowsRegistryCurrentControlSet, WindowsRegistryFilesAndTransactionLogs, WindowsRegistryProfiles, WindowsReleaseIdentifier, WindowsSearchDatabase, WindowsShellRunasCommand, WindowsSetupApiLogs, WindowsSuperFetchFiles, WindowsSystemRegistryFiles, WindowsSystemRegistryTransactionLogFiles, WindowsSystemRegistryFilesAndTransactionLogs, WindowsSystemResourceUsageMonitorDatabaseFile, WindowsTerminalServerRunKeys, WindowsTimezone, WindowsUninstallKeys, WindowsUpdateBuildRevision, WindowsUpdateStatus, WindowsUserAutomaticDestinationsJumpLists, WindowsUserCustomDestinationsJumpLists, WindowsUserDownloadsDirectory, WindowsUserJumpLists, WindowsUserRecentFiles, WindowsUserRegistryFiles, WindowsUserRegistryTransactionLogFiles, WindowsUserRegistryFilesAndTransactionLogs',
                             '--no_vss', '--hashers', 'None', storage_path, self.path, p_name])
    ...
    def Analyze_Documents(self):
        ...
        for par in par_info:
            # Get Document List
            query = "SELECT * FROM file_info WHERE extension in ('pdf', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'hwp') and par_id='" + \
                    par[0] + "' ORDER BY file_id;"
            ...
            if len(document_files):
                ...
                data.work_dir = "/home/carpe/defa_temp" + os.sep + self.case_id + os.sep + self.evd_id + os.sep + par[
                    0] + os.sep + "documents/"
                if not os.path.exists(data.work_dir):
                    os.mkdir(data.work_dir)

                data.ole_path = "/home/carpe/defa_temp" + os.sep + self.case_id + os.sep + self.evd_id + os.sep + par[
                    0] + os.sep + "documents/"

...
```
2. PARAMETER
- Managemet Class의 기본 파라미터들을 설정합니다. 이 파라미터 값들은 동 레벨의 다른 모듈과 공유하지 않으므로 set_attrib()를 사용하지 않았습니다. "dest"와 "confing" 파라미터를 각각 PARAMETER 인자에 추가하면 추출될 위치를 지시할 수 있고, 기본 config.txt가 아닌 다른 이름의 모듈 리스트가 적힌 설정 파일을 읽을 수 있습니다.
```python

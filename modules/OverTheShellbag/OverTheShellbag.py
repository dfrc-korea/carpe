...
def RefineResultToTuple(_parsing_result, _Bagkey):
  shell_item_result_key      = ["shell_type", "value", "fat_mtime", "file_size"]
  extension_block_result_key = ["extension_sig", "mtime", "atime", "ctime", "filesystem", "mft_entry_number", "mft_sequence_number"]
  ...
  return bag_path, shell_type, value, str(mtime), str(atime), str(ctime), str(last_written_time), file_size, mft_entry_number, mft_sequence_number, filesystem
  ...
  for SID in reg_data.keys():
    ...
    for idx, key in enumerate(value_data_result.keys()):
      ...
      if result:
        ...
        result_tuple = RefineResultToTuple(result, key)

        shellbags.append(result_tuple)

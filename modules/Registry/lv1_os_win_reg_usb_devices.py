...
def USBDEVICES(reg_software, reg_system, text_data):
    ...
    try:
        ...
        for device_key in device_key_list1:
            for device_class_id in device_key.subkeys():
                for serial_number in device_class_id.subkeys():
                    ...
                    device_list[device_count].last_connected_time = serial_number.last_written_timestamp().isoformat()+'Z'
                            ...
                            if '83da6326' in properties_subkey.name():
                                for properties_time_data in properties_subkey.subkeys():
                                    if properties_time_data.name() == '0064':
                                        device_list[device_count].driver_install_time = properties_time_data.last_written_timestamp().isoformat()+'Z'
                                    elif properties_time_data.name() == '0065':
                                        device_list[device_count].first_install_time = properties_time_data.last_written_timestamp().isoformat()+'Z'
                                    elif properties_time_data.name() == '0066':
                                        device_list[device_count].last_insertion_time = properties_time_data.last_written_timestamp().isoformat()+'Z'
                                    elif properties_time_data.name() == '0067':
                                        device_list[device_count].last_removal_time = properties_time_data.last_written_timestamp().isoformat()+'Z'
                ...
                if device_key != None:
                    for device_class_id in device_key.subkeys():
                        ...
                        if device_key == device_key_list2[0]:
                            ...
                            device_list[device_count].source_location = []
                            device_list[device_count].source_location.append('SYSTEM-ControlSet001/Enum/STORAGE/VolumeSnapshot')
                        ...
                        if '??' in device_class_id.name():
                            ...
                            device_list[device_count].first_connected_since_reboot_time = device_class_id.last_written_timestamp().isoformat()+'Z'
                        ...
                        for device_class_id_value in device_class_id.values():
                            if device_class_id_value.name() == 'DeviceDesc':
                                device_list[device_count].device_description = device_class_id_value.data().replace('\x00','')
                            ...
                            for reg_value_drive_letter in reg_key.values():
                                if 'DosDevices' in reg_value_drive_letter.name():
                                    if reg_value.data() == reg_value_drive_letter.data():
                                        device.last_assigned_drive_letter = reg_value_drive_letter.name().split('\\')[2].replace('\x00','')
        ...
        # first_connected_time - SETUPAPI
        for device_class_id in str(text_data).split(">>>  [Device Install (Hardware initiated) - SWD\\\\WPDBUSENUM\\\\"):

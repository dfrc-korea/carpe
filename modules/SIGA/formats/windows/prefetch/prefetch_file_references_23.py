...
class PrefetchFileReferences23(KaitaiStruct):
    ...
    def _read(self):
        ...
        self.number_file_references = self._io.read_u4le()
        self.array_file_references = [None] * (self.number_file_references)
        for i in range(self.number_file_references):
            self.array_file_references[i] = self._root.FileRef(self._io, self, self._root)

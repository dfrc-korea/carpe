...
def getVolumeHeader(data, stpos, fileinfo):
  ...
  try:
    fileinfo['Time'] = superfetch.filetime_to_datetime(ft, 9).isoformat()
  except OSError:
    fileinfo['Time'] = ''
  pos += 8   # VolumeId 시작점
...
def main(file_name):
  ...
  if proc_n == (0xE, 0x1, 0x48):    pos = getVolumeHeader(data, pos, fileinfo)          # 대상파일 : AgGl*History.db  (Windows 7)
  elif proc_n == (0xE, 0x1, 0x38):  pos = getVolumeHeader_0xExx38(data, pos, fileinfo)  # 대상파일 : AgGl*History.db  (Windows 7 x86)
  elif proc_n == (0xE, 0xB, 0x48):  pos = getVolumeHeader(data, pos, fileinfo)          # 대상파일 : AgCx_SC?.db      (Windows 7)
  elif proc_n == (0xE, 0xB, 0x38):  pos = getVolumeHeader_0xExx38(data, pos, fileinfo)  # 대상파일 : AgGlUAD_*.db     (Windows 7 x86)
  elif proc_n == (0x3, 0x15, 0x60): pos = getVolumeHeader(data, pos, fileinfo)          # 대상파일 : AgGl~History.db
  elif proc_n == (0x3, 0x0B, 0x60): pos = getVolumeHeader(data, pos, fileinfo)          # 대상파일 : AgCx_SC?.db
  elif proc_n == (0x3, 0x13, 0x60): pos = getVolumeHeader(data, pos, fileinfo)          # 대상파일 : dynrespri.7db, cadrespri.7db
    ...
    if p != -1:
      ...
      pos += len(b)
  elif proc_n == (0xE, 0xE, 0x38):  pos = getVolumeHeader_0xExx38(data, pos, fileinfo)  # 대상파일 : AgRobust.db  (Windows 7 x86)
  elif proc_n == (0xE, 0xE, 0x48):  pos = getVolumeHeader(data, pos, fileinfo)          # 대상파일 : AgRobust.db  (Windows 7)
  elif proc_n == (0xF, 0xE, 0x48):  pos = getVolumeHeader(data, pos, fileinfo)          # 대상파일 : AgRobust.db  (Windows 8 이상)
  elif proc_n == (0xF, 0x0, 0x48):  pos = getVolumeHeader(data, pos, fileinfo)          # 대상파일 : dynreservedpri.db  (Windows 8)
  else: 
    print('Error: Unknown format')
    exit()

  # db이름
  if (proc_n[0] == 0xE) and (proc_n[1] == 0x1): fileinfo['Name'] = data[244:data.find(b'\x00', 244)].decode('utf8')  
  elif (proc_n[0] == 0x3) and (proc_n[1] == 0x15): fileinfo['Name'] = data[252:data.find(b'\x00', 252)].decode('utf8')

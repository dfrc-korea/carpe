...
class JUMPLISTConnector(interface.ModuleConnector):
    ...
    DESCRIPTION = 'Moudle for Jumplist'
    ...
    def Connect(self, par_id, configuration, source_path_spec, knowledge_base):
        ...
        query = f"SELECT name, parent_path, extension FROM file_info WHERE par_id like '{par_id}' and " \
                f"extension like 'automaticDestinations-ms' and parent_path like 'root/Users/%/AppData/Roaming/Microsoft/Windows/Recent/AutomaticDestinations';"

        jumplist_automatic_files = configuration.cursor.execute_query_mul(query)

        query = f"SELECT name, parent_path, extension FROM file_info WHERE par_id like '{par_id}' and " \
                f"extension like 'customDestinations-ms' and parent_path like 'root/Users/%/AppData/Roaming/Microsoft/Windows/Recent/CustomDestinations';"
        ...
        for jumplist_automatic_file in jumplist_automatic_files:
            file_path = jumplist_automatic_file[1][jumplist_automatic_file[1].find('/'):] + '/' + \
                        jumplist_automatic_file[0]  # document full path
            fileExt = jumplist_automatic_file[2]
            fileName = jumplist_automatic_file[0]
            ...
            for idx, result in enumerate(results['DestList']):
                ...
                insert_jumplist_automatic_file.append(tuple([par_id, configuration.case_id, configuration.evidence_id,
                                                             result[5][result[5].rfind('\\') + 1:].replace('\\', '/'),
                                                             result[5].replace('\\', '/'),
                                                             str(result[1]).replace(' ', 'T') + 'Z', result[2],
        ...
        for jumplist_custom_file in jumplist_custom_files:
            file_path = jumplist_custom_file[1][jumplist_custom_file[1].find('/'):] + '/' + jumplist_custom_file[0]  # document full path
            fileExt = jumplist_custom_file[2]
            fileName = jumplist_custom_file[0]
            ...
            for idx, result in enumerate(results['LnkData']):
                if idx == 0:
                    continue
                insert_jumplist_custom_file.append(tuple([par_id, configuration.case_id, configuration.evidence_id, fileName, result[0], result[1], result[2], result[3], result[4]]))

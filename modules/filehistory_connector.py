...
class FileHistoryConnector(interface.ModuleConnector):
	...
	def Connect(self, configuration, source_path_spec, knowledge_base):
		...
		print('[MODULE]: FileHistory Analyzer Start! - partition ID(%s)' % par_id)
		...
		for idx, result in enumerate(results['namespace']):
			...
			insert_filehistory_namespace.append(tuple([par_id, configuration.case_id, configuration.evidence_id, str(result[0]),
											   str(result[1]), str(result[2]), str(result[3]), str(result[4]), file_created_time, file_modified_time,
											   str(result[7]), str(result[8]), str(result[9]), str(result[10])]))

		for idx, result in enumerate(results['file']):
			if idx == 0:
				continue
			insert_filehistory_file.append(tuple([par_id, configuration.case_id, configuration.evidence_id, str(result[0]),
											   str(result[1]), str(result[2]), str(result[3]), str(result[4]), str(result[5]), str(result[6]), str(result[7]), str(result[8])]))

		for idx, result in enumerate(results['string']):
			if idx == 0:
				continue
			insert_filehistory_string.append(tuple([par_id, configuration.case_id, configuration.evidence_id, str(result[0]),
											   str(result[1])]))

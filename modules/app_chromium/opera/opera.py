...
def opera_visit_urls(file):
    ...
    for row in result:
        ...
        visit_count = row[2]
        typed_count = row[3]
        last_visit_time = _convert_timestamp(row[4])
        hidden = row[5]
        activity_time = _count_microseconds(row[6])
        display_time = _count_microseconds(row[7])
        open_time = _count_microseconds(row[8])
        last_display_time = _convert_timestamp(row[9])
...
def opera_visit_history(file):
    ...
    try:
        ...
        print("[Web/Opera] Visit History " + "\033[31m" + "id-url dict query error" + "\033[0m")
        id_url_dict = {}
    ...
    try:
        ...
        print("[Web/Opera] Visit History " + "\033[31m" + "id-title dict query error" + "\033[0m")
        id_title_dict = {}
    ...
    try:
        ...
        print("[Web/Opera] Visit History " + "\033[31m" + "id-url-from-visits query error" + "\033[0m")
        id_url_visits = []
...
def opera_top_sites(file):
    ...
    # History SQLite DB에 visit_urls에 맞는 쿼리 날리기
    try:
...
def opera_logindata(file):
    ...
    try:
        ...
        print("[Web/Opera] Login Data " + "\033[31m" + "column check query error" + "\033[0m")
        result = []
